...
- 空闲轮询退避：无活动循环时客户端轮询退到 5 分钟，出现活动循环即回 60s；后台标签页整段暂停轮询（visibilitychange）。
- 事件台账：append-only 的 plugins-data/dsh-loop/loops.history.jsonl 记录 created/stopped/swept/remounted 四类件及循环元数据（v0.4.2）。
